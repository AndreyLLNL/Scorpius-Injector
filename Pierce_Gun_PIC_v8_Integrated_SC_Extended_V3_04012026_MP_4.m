...
fprintf('\n=== Pierce_Gun_PIC_v8_Integrated_SC_Extended_V3_04012026_MP_4.m ===\n');
...
%% Wall line
yline(sc_r(end)*1000, 'w-', 'LineWidth', 2.0, ...
      'HandleVisibility','off');
text(sc_z(end)*1000*0.04, sc_r(end)*1000*0.88, 'Wall', ...
     'Color','white','FontSize',11,'FontWeight','bold', ...
     'Interpreter','none');
...
if ENABLE_MULTIPULSE == true && ...
   ...
   ~isempty(ion_diag.snapshot_grids)
    ...
    for ip = 1:n_snap
        ...
        %% Wall reference
        yline(sc_r(end)*1000, 'w-', 'LineWidth', 1.8, ...
              'HandleVisibility','off');
        ...
        text(0.5, 0.97, sprintf('%.3e ions', n_ions_snap), ...
             'Units','normalized','HorizontalAlignment','center', ...
...
if ENABLE_ION_ACCUMULATION == true && sum(ion_density_grid(:)) > 10
    ...
    %sgtitle(sprintf('Ion Spatial Distribution  |  %d Pulse(s)  |  Total: %.3e ions  |  P=%.1e mbar  |  %s', ...
    ...
    text(0.01, 0.04, ...
         sprintf('Total: %.3e ions  |  Peak: %.2e at z=%.0f mm  |  Ion centroid r: %.1f mm', ...
...
if ENABLE_ION_ACCUMULATION == true && sum(ion_density_grid(:)) > 10
    ...
    text(z_end*0.98, r_top - 1.5, 'Wall (75mm)', ...
         ...
         'HorizontalAlignment','right','VerticalAlignment','top', ...
         'Interpreter','none');
    text(z_end*0.98, 48.5, 'Target (50mm)', ...
         'Color','magenta','FontSize',12, ...
         'HorizontalAlignment','right','VerticalAlignment','top', ...
         'Interpreter','none');
    ...
    text(0.01, 0.97, ...
         sprintf('Total: %.3e ions  |  Peak: %.2e at z=%.0f mm, r=%.0f mm  |  Ion centroid r: %.1f mm', ...
                 total_ions, peak_val, ...
...
if exist('steady_state_beam_p3', 'var') && steady_state_beam_p3.n_total > 1000
    ...
    for ip = 1:N_ANALYSIS_PLANES
        ...
        dz_window = steady_state_beam_p3.vz * dt * 30;
        in_plane = abs(steady_state_beam_p3.z - z_plane) < abs(dz_window);
        
        if sum(in_plane) < 100
            in_plane = abs(steady_state_beam_p3.z - z_plane) < 0.020;
        end
        
        n_selected = sum(in_plane);
        
        if n_selected > 50
            r_sel = steady_state_beam_p3.r(in_plane);
            pr_sel = steady_state_beam_p3.pr(in_plane);
            pz_sel = steady_state_beam_p3.pz(in_plane);
            ptheta_sel = steady_state_beam_p3.ptheta(in_plane);
            gamma_sel = steady_state_beam_p3.gamma(in_plane);
      ...
      fprintf('P3 analysis complete: %d locations processed\n', N_ANALYSIS_PLANES);
end
...
if exist('steady_state_beam_p4', 'var') && steady_state_beam_p4.n_total > 1000
    ...
    for ip = 1:N_ANALYSIS_PLANES
        ...
        dz_window = steady_state_beam_p4.vz * dt * 30;
        in_plane = abs(steady_state_beam_p4.z - z_plane) < abs(dz_window);
        
        if sum(in_plane) < 100
            in_plane = abs(steady_state_beam_p4.z - z_plane) < 0.020;
        end
        
        n_selected = sum(in_plane);
        
        if n_selected > 50
            r_sel = steady_state_beam_p4.r(in_plane);
            pr_sel = steady_state_beam_p4.pr(in_plane);
            pz_sel = steady_state_beam_p4.pz(in_plane);
            ptheta_sel = steady_state_beam_p4.ptheta(in_plane);
            gamma_sel = steady_state_beam_p4.gamma(in_plane);
      ...
      fprintf('P4 analysis complete: %d locations processed\n', N_ANALYSIS_PLANES);
end
...
if exist('twiss_results', 'var') && exist('twiss_results_p2', 'var')
    fprintf('\n=== GENERATING PULSE COMPARISON PLOTS ===\n');
    ...
    gamma_p1 = [twiss_results.gamma_tw];
    emit_p1 = [twiss_results.emit_norm];
    
    % Pulse 2 parameters
    beta_p2 = [twiss_results_p2.beta];
    alpha_p2 = [twiss_results_p2.alpha];
    gamma_p2 = [twiss_results_p2.gamma_tw];
    emit_p2 = [twiss_results_p2.emit_norm];
